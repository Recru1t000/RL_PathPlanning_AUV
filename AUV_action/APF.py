...
class Artificial_Potential_Field():
    def __init__(self,base_map,att_force=1.0,rep_force=100.0,rep_range=2.0,step_size=1.0):
        ...
        self.goal = np.array(self.base_map.set_goal_point()[0])
            ...
            if len(up_points)!=0 and abs(self.initial_point[1]-obstacle.get_obstacle()[2][1])<=self.rep_range:
                for up_point in up_points:
                    if up_point[0][0]<=self.initial_point[0]<=up_point[len(up_point)-1][0]:
                        dist = np.linalg.norm(self.initial_point-np.array([self.initial_point[0],obstacle.get_obstacle()[2][1]]))
                        rep_force += self.rep_force * ((1 / dist - 1 / self.rep_range) / dist ** 2) * (self.initial_point - np.array([self.initial_point[0],obstacle.get_obstacle()[2][1]]))
            ...
            if len(bottom_points)!=0 and abs(self.initial_point[1]-obstacle.get_obstacle()[0][1])<=self.rep_range:
                for bottom_point in bottom_points:
                    if bottom_point[0][0]<=self.initial_point[0]<=bottom_point[len(bottom_point)-1][0]:
                        dist = np.linalg.norm(self.initial_point-np.array([self.initial_point[0],obstacle.get_obstacle()[0][1]]))
                        rep_force += self.rep_force * ((1 / dist - 1 / self.rep_range) / dist ** 2) * (self.initial_point - np.array([self.initial_point[0],obstacle.get_obstacle()[0][1]]))
            ...
            if len(left_points)!=0 and abs(self.initial_point[0]-obstacle.get_obstacle()[0][0])<=self.rep_range:
                for left_point in left_points:
                    if left_point[0][1]<=self.initial_point[1]<=left_point[len(left_point)-1][1]:
                        dist = np.linalg.norm(self.initial_point-np.array([obstacle.get_obstacle()[0][0],self.initial_point[0]]))
                        rep_force += self.rep_force * ((1 / dist - 1 / self.rep_range) / dist ** 2) * (self.initial_point - np.array([obstacle.get_obstacle()[0][0],self.initial_point[0]]))
            ...
            if len(right_points)!=0 and abs(self.initial_point[0]-obstacle.get_obstacle()[1][0])<=self.rep_range:
                for right_point in right_points:
                    if right_point[0][1]<=self.initial_point[1]<=right_point[len(right_point)-1][1]:
                        dist = np.linalg.norm(self.initial_point-np.array([obstacle.get_obstacle()[2][0],self.initial_point[0]]))
                        rep_force += self.rep_force * ((1 / dist - 1 / self.rep_range) / dist ** 2) * (self.initial_point - np.array([obstacle.get_obstacle()[2][0],self.initial_point[0]]))
        return rep_force
